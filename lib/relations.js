...
export const relations = {
  ...
  // vendorContact / tenantContact: titleField/badgeField are NOT here —
  // TaskDetail's contactTitle and contactPropCode close over local
  // vendors/tenants state, so they stay caller-supplied props at the call
  // site (see CLAUDE.md Canonical Linker Architecture note, 2026-07-21).
  // Both entries are identical in every registry-able field — confirmed by
  // reading each call site independently, not assumed from symmetry.
  vendorContact: {
  ...
  // taskContacts: multi-mode (task_contacts join table), single call site
  // (TaskDetail Contacts section). linkedFields differs from vendorContact/
  // tenantContact — includes category + created_at, read fresh from the live
  // block rather than assumed. titleField/badgeField stay caller-supplied
  // (same contactTitle/contactPropCode closures as Vendor/Tenant Contact).
  // createFields + onCreate (the inline create-new-contact flow) stay local
  // too — tightly coupled write-side behavior, not query/display config.
  taskContacts: {
  ...
  // taskContacts' contactTitle/contactPropCode which closed over component
  ...
  // contactTenantCompany's prop_code pill. Both prop_code and status were
  // already in linkedFields, so this is display-only, no query change.
  keySafe: {
    mode: 'single',
    linkedTable: 'key_safes',
    linkedFields: 'id,key_safe_code,on_site_location,status,prop_code',
    searchFields: ['key_safe_code', 'on_site_location'],
