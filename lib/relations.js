...
export const relations = {
  ...
  //     NOT a bare "tenant_id.not.is.null" string and NOT a "prop_code=eq.X"
  //     filter on contacts. `contacts` has no `prop_code` column of its own
  //     (confirmed by reading every place this codebase queries `contacts`),
  //     so property-scoping has to go through the tenant ids that actually
  //     belong to that property — see the helper's own comment for the full
  //     reasoning, including why LinkField.jsx's searchFilter must always be
  //     a real `key=value` PostgREST fragment (a bare "column.op.val" string
  //     with no "=" silently filters nothing).
