// TEMPORARY — diagnostic only, no auth. Delete next session.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Step 1 — backfill bodies for existing messages with body_stored = false
    ...
    for (const msg of (messages || [])) {
      try {
        ...
        console.log(`[debug-sync] backfill failed for ${msg.gmail_message_id}:`, err?.message);
      }
    }

    // Step 2 — inbox poll: insert any messages not already in DB
    ...
    return res.status(200).json({ backfilled, synced });
...
async function processNewMessage(gmailClient, sb, account, gmailMessageId) {
  ...
  try {
    ...
    console.log(`[debug-sync] body fetch failed for ${gmailMessageId}:`, err?.message);
  }
