...
- **"Company doesn't auto-populate when a Contact is picked" bug (fixed 2026-07-22, root cause confirmed live, not just theorized):** `CompanyLinkCard` looks up `data.vendor_id`/`data.tenant_id` against the bulk `vendors`/`tenants` lists TaskDetail fetches once on mount (`select=id,company_dba,podio_id&order=...`, silently `.catch(()=>{})`'d). If that bulk fetch fails, races, or is simply slow, `.find()` permanently returns `undefined` with no retry — even though `tasks.vendor_id`/`tenant_id` saved correctly (confirmed via the actual PATCH body in a live-mocked reproduction: `handleContactChange`/`saveMany` write the FK fine; only the separate, decoupled local-list lookup was broken). This is why Vendor/Tenant **Contact** appeared to work (it resolves via LinkField's own targeted per-id fetch) while **Company** silently showed "—". **Verified this exact mechanism live** via a temporary diagnostic page + Playwright with Supabase REST calls intercepted/mocked (no real DB access available in-session either) — reproduced the failure by forcing the bulk `vendors` GET to 500, confirmed the fix recovers it, both for a freshly-picked contact and for an existing WO with the FK already set at mount (no user interaction). **Fix:** two small self-healing `useEffect`s (keyed on `data.vendor_id`/`data.tenant_id`) that fetch just the one missing row by id and merge it into local state whenever the FK is set but not already present in the bulk list — a single-row fetch is far less likely to fail than the full list, independent of why the bulk fetch didn't have it. No change to `CompanyLinkCard` itself, no change to the working case (bulk fetch succeeding stays a no-op).
...
- **"Contacts section doesn't render on iOS" bug (fixed 2026-07-22, root cause confirmed live — pre-existing since the `taskContacts` multi-mode migration on 2026-07-21, NOT a regression from this session's Key Safe/Alert/Vendor-Company work, confirmed via `git log -S` on `rel="taskContacts"`):** `LinkField.jsx`'s multi-mode `loadLinked()` (the fetch behind every multi-mode `RelationField`, not just Contacts — Related Records, reverseFK Vendor/Tenant Contacts tabs, etc.) had a bare `catch { setLinked([]) }` with no retry. A single transient network failure (confirmed live by forcing the `task_contacts` join-table fetch to fail once) leaves the section's outer card/header rendering normally but permanently empty — no cards, no visible error, no way to recover short of a full reload. This reads exactly like "the section doesn't render," even though the outer JSX (header + Plus button) was never conditional and was never actually missing — confirmed by inspecting the full rendered page, not assumed. **Fix, in `LinkField.jsx` itself (not per-call-site) per the Canonical Linker Architecture rule — benefits every multi-mode relationship, not just Contacts:** `loadLinked()` split into `fetchLinkedOnce()` + a wrapper that retries once after an 800ms delay before giving up. Verified live: forcing the first `task_contacts` fetch to fail then succeed on retry correctly populates the section; forcing every attempt to fail still degrades to the pre-existing empty-with-no-error state (unchanged, not a regression — a permanently offline case was never going to self-heal either way). Investigated the same way as the Vendor/Tenant Company bug earlier this session: a temporary diagnostic page + Playwright with Supabase REST calls intercepted/mocked (this session has no direct DB network access), since a platform-specific rendering failure needed live reproduction, not another code-only read.
