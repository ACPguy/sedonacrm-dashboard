...
**Registry entries and their call sites** (all migrated 2026-07-21/22; general purity rule above governs what's local vs registry for each):
- `property` — TaskDetail + NewTaskForm. All-pure config; `onChange` (saves `property_id`+`prop_code` together) stays local at both sites.
- `vendorContact` / `tenantContact` — TaskDetail Linked Companies. `titleField`/`badgeField` stay local (close over `vendors`/`tenants` state via `contactTitle`/`contactPropCode`). Tenant Contact modal deliberately does not auto-create a `tenants` row.
- `taskContacts` — TaskDetail Contacts (multi-mode, `task_contacts` join table). `joinTable`/`parentIdField`/`linkedIdField` live in the registry; `parentId`, `titleField`/`badgeField`, `createFields`+`onCreate` stay local.
- `relatedRecords` — TaskDetail Related Records (`task_relations`, self-referential tasks↔tasks, `titleTarget:'_self'`). `searchFilter`/`titleHref` stay local (depend on the CURRENT record, not `row` — this is the 4069921 same-tab nav + `?rt=`/`?from=` fix). `titleField`/`iconField` are pure, live in registry.
...
- `contactLinkedTasks` — ContactDetail's read-only reverse view of `task_contacts` (opposite direction from `taskContacts`: `parentIdField:'contact_id'`). `readOnly:true` baked in.
- `taskVendorCompany` / `taskTenantCompany` — NewTaskForm WO section (`mode:'single'`, writes `tasks.vendor_id`/`tenant_id` directly — a different FK than `contactVendorCompany`/`vendorContact`). Query/display shape identical to `contactVendorCompany`/`contactTenantCompany`, kept as separate entries for call-site clarity. **2026-07-22: removed from NewTaskForm** — see Next Priorities / Section 1 note below; entries remain in `relations.js` for potential future TaskDetail use.
- `keySafe` — TaskDetail + NewTaskForm WO Details (`mode:'single'`, writes `tasks.key_safe_id`). `titleField` leads with prop_code, `badgeField` shows status, `metaField` shows contents (required removing LinkField's single-mode `!compact` gate on `metaField`).
...
- **Linked Companies:** uses `LinkField mode='single'` for both Vendor Contact and Tenant Contact. Picking a contact auto-fills the read-only Company box via `handleContactChange` (reads `vendor_id`/`tenant_id` FK from the contact row, saves both fields via `saveMany`). "+ Create new" opens `StackedFormModal` (zIndex 310) with a 4-field form (name/company/phone/email). **Deliberate asymmetry:** Vendor Contact modal auto-creates (or reuses via case-insensitive exact ilike match on company_dba) a `vendors` row when a company name is typed. Tenant Contact modal intentionally does NOT create a `tenants` row — new tenants must go through the leasing pipeline.
- **Vendor/Tenant Company lookups** (TasksList rows, TaskDetail display) load ALL vendors/tenants regardless of status — do NOT add an Active-only filter back (a prior version's Active-only filter silently blanked out ~74%/65% of correctly-linked companies; fixed 2026-07-20).
- **Vendor Company / Tenant Company fields** (Linked Companies card, TaskDetail only — see Section 1 note below for NewTaskForm) rendered via `CompanyLinkCard` — icon (Truck/Storefront) + clickable company name with ↗ + optional badge. Blank state shows '—'. Do not re-inline this JSX. Self-heals via a per-id fetch merged into local state whenever `data.vendor_id`/`tenant_id` is set but missing from the bulk-fetched list (fixed 2026-07-22 — bulk fetch failure/race left `.find()` permanently `undefined` even though the FK itself saved fine).
- **Tenant Company** shows a `prop_code` badge. Vendor Company intentionally has no badge — vendors aren't tied to a single property.
- **Contacts, Vendor Contact, Tenant Contact trigger buttons** are all `Plus` icon (14px) with a `title` tooltip and `excludeRef` wired to their own button.
...
- **CompanyLinkCard icon** is 32px to match the unified contact icon size.
- **Alert field removed from Follow-Up** (TaskDetail + NewTaskForm) — UI only, `tasks.alert` column untouched.
- **"Keys / Key Safe" free-text field (`tasks.key_safe_info`)** hidden from both TaskDetail's and NewTaskForm's WO sections (2026-07-22) — UI only, column untouched. Both forms use the `keySafe` RelationField instead (`mode:'single'`, writes `key_safe_id`), scoped by `searchFilter={prop_code.eq.<prop_code>}` and only rendered inside `record_type==='work_order'`. See Next Priorities for the still-open Podio-sync mapping gap.
- **NewTaskForm's Vendor/Tenant Company fields removed entirely (2026-07-22, Scott's explicit call):** rather than a combined header, Vendor Company and Tenant Company are hidden from NewTaskForm until the task is saved (no `id` exists yet for the old company-first cascade to narrow against). `CompanyContactRow` (the old company+contact combo, company-first search narrowing) was deleted — replaced by `NewTaskContactField` (Contact-only, unscoped search except where `searchFilter` is explicitly passed) + `AvailableAfterSaving` (shared placeholder, styled to match LinkField's own single-mode empty-state box: `fontSize:F.sm, color:T.text3, italic, T.bg3 bg, 0.5px T.border, radius 6px`). Same placeholder now also fills the previously-silent-and-absent Contacts and Related Records sections in NewTaskForm (both need a real task `id`; they simply didn't render before, which read as broken rather than intentional). TaskDetail's own Vendor/Tenant Company + Contacts + Related Records are unchanged — this only affects the create form. `taskVendorCompany`/`taskTenantCompany` registry entries stay in `lib/relations.js` (unused by NewTaskForm now, available if TaskDetail or a future call site needs them). NewTaskForm's dead `vendorContacts`/`tenantContacts` state+fetches (only ever fed the old cascade's `allContacts` prop) were deleted along with `CompanyContactRow`.
- **NewTaskForm's hardcoded `isMobile={false}` fixed (2026-07-22):** NewTaskForm now has its own `isMobile` state + resize listener, identical to TaskDetail's pattern (`window.innerWidth<640`, updated on `resize`). Used by the Vendor/Tenant Contact + Company row wrappers for mobile stacking.
- **Tenant Contact search scoped to the selected property (2026-07-22):** both TaskDetail and NewTaskForm now pass `searchFilter={prop_code.eq.<prop_code>}` to Tenant Contact's RelationField, matching the pattern Key Safe already used. Vendor Contact is deliberately NOT scoped — vendors work across multiple properties.
- **Key Safe "disappears when saved empty" — investigated live 2026-07-22, could NOT reproduce.** Reported: Key Safe shows in NewTaskForm while creating, but doesn't appear in TaskDetail after saving a WO with it left blank. Tested via 4 independent live reproductions (mocked Supabase REST + real Chromium, matching the project's established live-verification technique): (1) direct TaskDetail load with `record_type:'work_order', key_safe_id:null, prop_code` set; (2) same with `prop_code:null`; (3) the actual real flow — fill NewTaskForm, click Save (client-side `router.push`, not a hard reload), land on the new task; (4) same as (3) at a 390px mobile viewport. In all four, the Key Safe field renders correctly with its empty `—` state, alongside WO Category/Budget Item/WO Type, confirmed via `innerText` (which excludes hidden/`display:none` content, so this isn't a false positive) and a screenshot. **No code change made for this finding** — the described mechanism doesn't exist in the current code at HEAD. Likely explanation: Scott's observation predates one of this session-period's earlier fixes (the `keySafe` linker was only added to NewTaskForm a few commits before this session, per git log), or was seen against a stale Vercel preview/browser cache. Recommend Scott re-verify once this session's commit is deployed and hard-refreshed; if it recurs, capture the exact record_type/key_safe_id/prop_code of the specific row for a targeted repro.
