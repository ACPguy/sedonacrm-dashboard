...
- `contact` — **consolidated 2026-07-23**, replacing the former separate `vendorContact`/`tenantContact`/`taskContacts` entries (all three had identical query/display config). One entry, three call-site shapes: Vendor Contact (TaskDetail + NewTaskForm, `mode:'single'`, `searchFilter="vendor_id=not.is.null"`), Tenant Contact (same, `searchFilter` built by `tenantIdsSearchFilterFor()` in TasksView.jsx — see the searchFilter syntax bug note above for why this can't just be a `prop_code` filter on `contacts`, which has no such column), and the plain multi-select Contacts list (TaskDetail Contacts section, `mode:'multi'` default, no searchFilter — any contact, any category). `titleField`/`badgeField` stay local at every call site (`contactTitle`/`contactPropCode`, closing over local `vendors`/`tenants` state). `createFields`+`onCreate` (multi-mode only) also stay local.
...
- `keySafe` — TaskDetail + NewTaskForm WO Details (`mode:'single'`, writes `tasks.key_safe_id`). `titleField` leads with prop_code, `badgeField` shows status, `metaField` shows contents (required removing LinkField's single-mode `!compact` gate on `metaField`). `searchFilter` fixed 2026-07-23 (see syntax bug note above).
...
- **NewTaskForm mirrors TaskDetail's layout exactly (2026-07-23):** the old `CompanyContactRow` (company-first cascade, added 2026-07-22 then already removed once before this) and its 247eadb-era replacement `NewTaskContactField`+`AvailableAfterSaving`-for-Company pairing are both gone. `NewTaskContactField` was generalized into `NewTaskLinkField` (adds a `sectionLabel` prop) and is now used for all 4 fields — Vendor Contact, Vendor Company, Tenant Contact, Tenant Company — each its own full-width block, never paired in a row. Company is pickable immediately during creation (writes into local `formData.vendor_id`/`tenant_id`, same as Property/Key Safe already did) — no more waiting for a saved task id. `AvailableAfterSaving` itself is unchanged and still used for Contacts (multi-list) and Related Records, which genuinely do need a real task id (join tables). NewTaskForm's now-unused `isMobile` state + resize listener (added 247eadb specifically for the old Contact+Company row-pairing's mobile stacking) was removed — the new one-field-per-row layout doesn't need a breakpoint check.
...
- **Tenant Contact search scoped to the selected property, correctly this time (2026-07-23):** `247eadb` (2026-07-22) first attempted this via `searchFilter={`prop_code.eq.${prop_code}`}`, which had two independent bugs — the missing `=` (see searchFilter syntax bug note above) AND `contacts` has no `prop_code` column of its own to filter on in the first place (property is only ever resolved via the `tenant_id` FK to `tenants.prop_code` — confirmed by reading every place this codebase queries `contacts`). Fixed via `tenantIdsSearchFilterFor(tenants, propCode)` (top of TasksView.jsx): derives the qualifying tenant ids from the already-loaded local `tenants` list and builds `` `tenant_id=in.(${ids.join(',')})` ``, falling back to a guaranteed-empty filter when no tenants match the property (never silently reverts to unscoped). Applied at both TaskDetail's and NewTaskForm's Tenant Contact call sites. Vendor Contact is deliberately NOT property-scoped (`searchFilter="vendor_id=not.is.null"` only) — vendors work across multiple properties.
