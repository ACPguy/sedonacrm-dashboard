...
- **Part 3 — DONE.** Per Scott's correction, no new buttons were created — the existing "Email Request To Vendor" field (TaskDetail only, not NewTaskForm — matches spec's "TaskDetail's WO section") was rewired in place. `GenericPills` itself was NOT modified (shared with WO Type/Invoice Location elsewhere) — instead a new local `EmailRequestPill` component (TasksView.jsx, right after `GenericPills`) renders the "No Files"/"With Files" options as action buttons labeled "Send Request - NO FILES" / "Send Request - WITH FILES" (display label only — the stored `email_request_sent` value stays the original `'No Files'`/`'With Files'` string, no schema/data change). "Discussed In-Person" stays exactly as before — still rendered via the real `GenericPills` component (now passed a single-option array) so its toggle/deselect behavior is unchanged.
  - **Flow:** `openRfpEmail(withFiles)` (TaskDetail) resolves Vendor Contact (`data.vendor_contact_id` → `contacts` row, needs a real `email`), Key Safe, Tenant Contact (attached as `task.tenantContact` per Part 2's caller contract), and property (read from the already-loaded `activeProps` list — no extra fetch) — then calls `buildRfpEmailHtml` and opens `EmailCompose` pre-filled (`prefilledTo`, `defaultSubject`, `initialBody`). "With Files" additionally calls the new `/api/tasks/list-attachments?folderId=` (Part 3b) first and passes the Drive file list into both the template's `attachmentList` and `EmailCompose`'s new `initialAttachments` prop. Guards (shown inline below the pills via `rfpError`, not a blocking modal): no Vendor Contact set, Vendor Contact has no email, or "With Files" clicked with no Drive folder on the task yet.
  - **`tasks.email_request_sent` is only set on a real, successful send** — inside `EmailCompose`'s `onSend` callback (fired after `/api/gmail/send` returns 200), never optimistically on click. Set to the clicked pill's own value (`'No Files'` / `'With Files'`), matching the existing 3-option field exactly — no new `'Sent'` value was introduced (supersedes the original spec text, which assumed a `'Sent'` value before Scott's correction).
  - **EmailCompose.jsx extended, not rebuilt:** new `initialAttachments` prop + `driveAttachments` state, rendered as chips in the existing attachment zone alongside (but visually distinguished, accent-colored border from) the pre-existing local drag-and-drop `files` chips. On Send, `attachments: driveAttachments.map(...)` `{driveFileId,filename,mimeType}` is added to the `/api/gmail/send` POST body. **Note found, not fixed (out of scope for this session):** the pre-existing local drag-and-drop `files` state was already dead functionality before this change — `handleSend`'s POST body never included it, so files dropped into the zone were silently never sent. Untouched here; only the new Drive-attachment path actually reaches the send API.
  - **`/api/tasks/list-attachments.js` (3b):** GET, `?folderId=`, read-only `drive.files.list()` (id/name/mimeType/size), same `scott@andersoncp.com` `email_accounts` row + `getDriveClient` pattern as `create-drive-folder.js`.
  - **`/api/gmail/send.js` (3c):** new optional `attachments` array `{driveFileId,filename,mimeType}`. No-attachments path is byte-identical to before (same `lines` array construction, verified by keeping the exact original 4 lines in the `!hasAttachments` branch). With attachments: fetches each file's bytes via `drive.files.get({fileId,alt:'media'},{responseType:'arraybuffer'})` (same Drive-scoped `account.id` already resolved for Gmail — one combined OAuth token, confirmed via `create-drive-folder.js`'s identical-account pattern), builds a real `multipart/mixed` MIME message by hand (boundary, base64 body wrapped at 76 chars per RFC 2045, `Content-Disposition: attachment`). Filenames sanitized (strip CR/LF, escape quotes) before landing in MIME headers.
  - **Verified live** via Playwright + mocked Supabase/API responses (network access to the real Supabase/Gmail/Drive isn't available in this environment) against a realistic WO fixture (vendor contact with email, tenant contact, key safe, Drive folder with 2 files): clicking each pill opens `EmailCompose` correctly pre-filled (TO/Subject/Body), the "With Files" compose shows both Drive attachment chips, the `/api/gmail/send` POST body carries the correct `to`/`subject`/`attachments` for each case, and `tasks.email_request_sent` PATCHes to the correct value (`'No Files'` then `'With Files'`) only after each send resolves — confirmed via the pill's active/highlighted state updating correctly in a follow-up screenshot. Test route/fixtures removed after verification; `package.json`/`package-lock.json` untouched (Playwright installed via `npm install --no-save`, uninstalled after).
