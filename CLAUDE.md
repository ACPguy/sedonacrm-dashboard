...
## RFP Vendor Email — DONE (3-part build, complete)

- `public/branding/acp-logo.png` — real logo. If it ever needs re-fetching, use Drive id `0BxXST_g3FJFkaGI1VWZNdi15R3M` ("ACP - Large Logo - transparent.png") — NOT `1yj87nkP50RRhNZot_h7Wbqg3cVJr3HB2`, which is a screenshot of a Podio PDF editor, not a usable logo.
- `lib/rfpEmailTemplate.js` — `buildRfpEmailHtml(task, property, vendorContact, keySafe, attachmentList)`. **Caller contract:** the Tenant section only renders if the caller attaches the resolved tenant contact onto the task object as `task.tenantContact = { full_name, primary_phone }` (`tasks.tenant_contact_id` is only an FK). `attachmentList` items need only a `name` field. `task.details` goes through a small inline sanitizer (strips script/style/iframe/object/embed/form, on*= handlers, javascript: URLs).
- TaskDetail's existing "Email Request To Vendor" field was rewired, not replaced with new buttons — `EmailRequestPill` (TasksView.jsx) renders "No Files"/"With Files" as action buttons that build+open the RFP email via `openRfpEmail(withFiles)`; `email_request_sent` keeps its original `'No Files'`/`'With Files'` values, no new `'Sent'` value. Only set on a real successful send (`EmailCompose`'s `onSend`, after `/api/gmail/send` returns 200), never optimistically. "Discussed In-Person" unchanged (`GenericPills`, untouched — shared with WO Type/Invoice Location).
- `/api/tasks/list-attachments.js` — GET `?folderId=`, read-only Drive `files.list()`.
- `/api/gmail/send.js` — optional `attachments: [{driveFileId,filename,mimeType}]`; no-attachments path byte-identical to before. With attachments: fetches bytes via `drive.files.get(...,{responseType:'arraybuffer'})`, builds a hand-rolled `multipart/mixed` MIME message.
- **Known dead code, not fixed (out of scope when found):** `EmailCompose`'s pre-existing local drag-and-drop `files` state was already never included in `handleSend`'s POST body before this build — files dropped into that zone are silently never sent. Only the new Drive-attachment path (`initialAttachments`/`driveAttachments`) actually reaches the send API.
...
**⚠️ searchFilter syntax bug (found + fixed 2026-07-23):** `LinkField.jsx` appends `searchFilter` as a raw `&${searchFilter}` string. A bare `"column.op.val"` string with no `=` is not a valid PostgREST filter fragment — it parses as a query param with an empty value, which PostgREST silently no-ops rather than filtering on. **Every `searchFilter` value must contain a literal `=`** — e.g. `vendor_id=not.is.null`, `prop_code=eq.ACP`, `tenant_id=in.(id1,id2)`. Fixed at Key Safe and Tenant Contact. **Still has this bug, deliberately unfixed:** `relatedRecords`' `` searchFilter={`id.neq.${data.id}`} `` at TaskDetail's Related Records — needs `` `id=neq.${data.id}` ``.
...
## Canonical Comms & Activity Sidebar Architecture (permanent)

`ActivityPanel` (`components/shared/ActivityPanel.jsx`) is the canonical source for the collapsible right-sidebar panel used on detail views — same "build once, use everywhere" tier as `LinkField.jsx`. Do NOT build a new sidebar/tab component for any future record type — extend this one and wire it into the new view instead. It's self-contained (own local `F`/card style, matching `LinkField.jsx`'s own pattern) rather than importing constants from `TasksView.jsx`, specifically to avoid a circular import (`TasksView.jsx` imports `ActivityPanel`, so `ActivityPanel` can't import back from it).

**Two tabs, Comms first/default:**
- **Comms** (renamed from "Comments" 2026-07-25, along with the internal state value `'comments'`→`'comms'`) — takes an optional `commsProps` prop (object of `CommunicationTimeline`'s own props: `recordType`, `recordId`, `fromAccount`, `crmRecordLabel`, `crmRecordUrl`). When provided, renders `<CommunicationTimeline {...commsProps}/>` (unpadded — the tab content wrapper's `12px` padding is conditionally dropped to `0` when showing the timeline, since `CommunicationTimeline` manages its own internal padding/layout and was previously given a bare wrapper too). When `commsProps` is omitted (no task/record to attach a timeline to), renders the original "Comments and files will sync from Podio via API at go-live" placeholder, unchanged.
- **Activity** (#2) — untouched, audit-purposes-only placeholder ("Activity tracking begins at go-live"), regardless of `commsProps`.

**Current consumers on the canonical version (2026-07-25):**
- `TasksView.jsx` (`TaskDetail`) — imports from `./shared/ActivityPanel`, passes `commsProps` (`recordType: data.record_type, recordId: data.id, fromAccount: "scott@andersoncp.com", crmRecordLabel, crmRecordUrl`). This replaced TaskDetail's old top-of-page "Details / Comms" tab bar entirely — Comms is no longer a separate main-content view swapped in/out via `detailTab` state; the Details form now always renders in the main column, and Comms lives exclusively in the sidebar (default tab, no extra click needed). `detailTab` state, the tab bar JSX, and the `{detailTab==='comms'&&(...)}` main-content `CommunicationTimeline` block are all gone; the now-unused `CommunicationTimeline` import was removed from `TasksView.jsx` too (its only remaining use in that file was that removed block).
- `KeySafesView.jsx` — imports from `./shared/ActivityPanel` (previously `import { ActivityPanel } from './TasksView'`). Does NOT pass `commsProps` (no task record to attach a timeline to) — keeps the original placeholder exactly as before. Side benefit confirmed via build output: `/key-safes/[id]`'s bundle size dropped noticeably, since it no longer pulls in the whole `TasksView.jsx` module graph just to get one component.

**Known duplicates NOT yet migrated (tracked follow-up, not attempted this session):** `WorkOrdersView.jsx` (dead code — the legacy `/work-orders` route, superseded by `TasksView.jsx`'s unified tasks table; see Current Component Structure), `IssuesView.jsx`, `SedonaCRM.jsx` (two separate inline copies), `VendorsView.jsx`'s `VdActivityPanel`, `OwnersView.jsx`'s `OwActivityPanel`. All five have their own fully-local `ActivityPanel`-shaped component (none import from `TasksView.jsx` or the new shared file), confirmed by reading each file before this migration — none of the two-file changes above touch them. Migrating each to the canonical component (and, where applicable, wiring `commsProps` the same way TaskDetail now does) is separate future work.

## TaskDetail Architecture Notes (permanent)

- **Main content column — 10 section cards (in order), always visible (no longer a "Details" tab — see Canonical Comms & Activity Sidebar Architecture for the 2026-07-25 removal of the top Details/Comms tab bar; Comms now lives exclusively in the sidebar):** Core → Follow-Up → Contacts (LinkField multi, all types) → Vendor / Tenant Contact (LinkField mode='single' ×2) → Vendor / Tenant Company (LinkField mode='single' ×2) → Related Records (LinkField multi, task_relations join table, all types) → Work Order Details (WO only; Financials + Closeout collapsed sub-panels) → Notes & Relationships → Documents → Dates. System Info collapsible block at end.
...
- **Contact/Company Decoupling Rebuild (2026-07-23, REVERSED 2026-07-25 — see next bullet)** — Contact and Company were made two fully independent fields on `tasks`; picking a Contact wrote ONLY `{type}_contact_id`, never `vendor_id`/`tenant_id`. Left here for history; superseded by the cascade below.
- **Contact→Company cascade restored (2026-07-25):** reverses the above. `handleContactChange` (TaskDetail) once again cascades — when a Contact is picked AND that contact has its own linked company (`row.vendor_id`/`row.tenant_id`), the task's `vendor_id`/`tenant_id` is updated in the SAME `saveMany` call as the `{type}_contact_id` write. If the picked contact has no linked company, the task's existing Company value is left alone (simply omitted from the PATCH, never explicitly nulled). Clearing the Contact (picking nothing) only clears `{type}_contact_id` — Company is never touched on clear. Manual Company picks are completely unchanged: `handleVendorCompanyChange`/`handleTenantCompanyChange` still just overwrite `vendor_id`/`tenant_id` directly, same as any other field edit — a manual pick after an auto-cascade simply overwrites it, no special-casing needed. NewTaskForm's `handleContactChangeForm` mirrors the same logic, staging into local `formData` via `set()` instead of `saveMany` (nothing persists until `handleSave`). This required changing `NewTaskLinkField`'s `onChange` contract from "receives just the picked id" to "receives the full row" (`onChange={onChange}` instead of `onChange={row=>onChange(row?row.id:null)}`) — safe because, after the next bullet's removal, it has exactly 2 remaining call sites (Vendor Contact, Tenant Contact), both needing row-level cascade logic. The `/api/tasks/create.js` first-save-only server fallback (still in place, unchanged) is now redundant with this client-side staging for the create path specifically, but harmless — it only fires when Company is still blank, which the client-side cascade already prevents whenever the picked contact has one.
- **Vendor/Tenant Company removed from NewTaskForm (2026-07-25):** TaskDetail keeps its own always-visible, independently-editable "Vendor / Tenant Company" card unchanged (still `taskVendorCompany`/`taskTenantCompany`, still manually pickable/overridable at any time). NewTaskForm no longer has a Company card at all — `formData.vendor_id`/`tenant_id` are still in its initial state and still sent in `handleSave`'s POST body (needed for the cascade above and for the server fallback), just no longer independently editable during creation. This makes the two forms asymmetric again for this one card — a deliberate call, not an oversight; see the "NewTaskForm history" bullet below for how NewTaskForm's Company card came to exist in the first place, before this removal.
- **NewTaskForm history (superseded — current state is the 2026-07-25 reorder bullet above, not this):** 2026-07-23 generalized `NewTaskContactField` into `NewTaskLinkField` and made Vendor/Tenant Contact+Company independently pickable during creation, ungated from `record_type==='work_order'`. 2026-07-24 added staged picking for Contacts/Related Records (`RelationField mode="staged"`, full row objects in local `stagedContacts`/`stagedRelatedRecords`, written for real via a POST loop in `handleSave` only after `/api/tasks/create` returns a real id — a link failure post-creation shows an error banner + "Open created task →" link rather than auto-navigating away) and reconciled NewTaskForm's card order to match TaskDetail's. The Vendor/Tenant Company card this all describes was later removed entirely (2026-07-25, see above) — Company is TaskDetail-only now.
...
- **"Keys / Key Safe" free-text field (`tasks.key_safe_info`)** hidden from both TaskDetail's and NewTaskForm's WO sections (2026-07-22) — UI only, column untouched. Both forms use the `keySafe` RelationField instead (`mode:'single'`, writes `key_safe_id`), scoped by `searchFilter={`prop_code=eq.${prop_code}`}` (fixed missing `=` 2026-07-23 — see Canonical Linker Architecture's searchFilter syntax bug note) and only rendered for `record_type==='work_order'`. See Next Priorities for the still-open Podio-sync mapping gap.
- **NewTaskForm reordered + Key Safe extracted to its own standalone card (2026-07-25):** top-to-bottom order is now Key Safe (new standalone card, WO-type only) → Vendor/Tenant Contact → Email Contacts (renamed from "Contacts", display label only — `rel`/refs/state all still `contact`/`stagedContacts`) → Related Records → Work Order Details (Key Safe row removed, everything else unchanged). Key Safe's card matches the label+Plus-button-header/field-row-below pattern (folded into one row since it's a single field); same `newKeySafeLinkRef`/`newKeySafeBtnRef`/`handleKeySafeChangeForm` as before, just relocated out of the WO Details FieldRow list. TaskDetail's own Key Safe field is unchanged — still inside its WO Details card.
- **4 legacy WO Financials fields hidden (2026-07-25, UI only):** Pmt Instructions to BK, Invoice Location, Invoice Stage, Invoice Paid removed from both TaskDetail's `woFinOpen` collapsible and NewTaskForm's Work Order Details. `pmt_instructions_to_bk`/`invoice_location`/`invoice_stage`/`invoice_paid` columns, `INVOICE_LOCATION_OPTIONS`/`INVOICE_STAGE_OPTIONS` constants, and `formData`'s initial state (still has all 4 as `null`) are untouched — historical-purposes-only removal, not a data deletion.
...
- **Key Safe "disappears when saved empty"** — investigated live 2026-07-22, could NOT reproduce across 4 independent live reproductions; no code change made. Likely a stale-cache observation. A DIFFERENT, real Key Safe bug (search not filtering by property) was found and fixed 2026-07-23 — see Canonical Linker Architecture's searchFilter syntax bug note. Two distinct reports; do not conflate them.
- **Field audit (2026-07-22):** TaskDetail-only fields with no NewTaskForm equivalent — Bid Status, Depends On Task #, Parent Project # / Project Type / Sequence Order (project-only), Close Date. Tracked in Next Priorities #9; no fixes applied.

## Current Git State

- main: `148d885` — merge: Contact/Company decoupling + searchFilter bug fixes + staged Contacts/Related Records + NewTaskForm field-order reconciliation (preview 00c32d7) into main (merged 2026-07-24, Scott approved: "Yes. Merge to main."). Brings in `247eadb`/`4cd6d4f`/`102e1ae` — see TaskDetail Architecture Notes and Canonical Linker Architecture for what changed. (Also includes an earlier same-day merge, `8c333b6`, of the Key Safe linker arc below — `8c333b6` is an ancestor of `148d885`, confirmed via `git merge-base`.)
- preview: `a3b5e83` — well ahead of main; not yet merged. Since the `148d885` merge, preview has picked up: the full Key Safe linker arc (card display, NewTaskForm linker, search expansion — `189b8ef`..`6ded972`..`4a1e71f`), the NewTaskForm Vendor/Tenant Contact+Company native-select→RelationField fix (`d1e128a`, fixed an iOS-unusable scroll-wheel picker), the Vendor/Tenant Company stale-bulk-fetch fix (`0d3c053`), the LinkField multi-mode retry-once fix (`4342e5d`), the Contact→Company cascade reversal + NewTaskForm Vendor/Tenant Company card removal + compact empty-state fix (`a670f19`), the WO Financials 4-field hide (`ffcf4c9`), the NewTaskForm reorder + Key Safe standalone card + Email Contacts rename (`dae080b`), and the ActivityPanel extraction to `components/shared/ActivityPanel.jsx` + Comms-into-sidebar migration (`a3b5e83`). See Canonical Comms & Activity Sidebar Architecture, Canonical Linker Architecture, and TaskDetail Architecture Notes for what each of these actually changed. `tasks_wo_type_check` DB constraint fix (Schema Notes) was run by Scott directly, not part of any commit.
...
- **`tasks_wo_type_check` constraint — fixed 2026-07-25, by Scott directly (not from any session):** New Work Order saves were failing this CHECK constraint even though `WO_TYPE_OPTIONS` (`['Work Order Request','Request For Proposal']`) was confirmed clean end-to-end — single source of truth, no whitespace/casing issue, no transformation between `GenericPills` and `/api/tasks/create.js`. Root cause (by elimination, since the code path was clean): the constraint's allowed-value list didn't match these two exact strings. No direct DB/psql/REST access was available from any session to inspect the constraint definition or existing `wo_type` values directly, so the diagnostic queries and fix were handed to Scott to run himself; confirmed resolved by Scott 2026-07-25. Exact ALTER text not recorded here — if this resurfaces, re-run `SELECT pg_get_constraintdef(oid) FROM pg_constraint WHERE conname = 'tasks_wo_type_check';` to see the current state first.
