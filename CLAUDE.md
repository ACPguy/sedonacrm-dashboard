...
- **New Inquiry agent (Agent 3) keyword filter was too broad 2026-07-03 through 2026-07-11** (substring matching, generic keywords, and later an over-broad automation.podio.com hard-include found during cleanup). Fixed 2026-07-11 with word-boundary phrase tiers, scoped hard-include/exclude rules, and known-contact flagging via contacts.email. 75 false-positive leasing_pipeline records deleted (`cleanup_agent3_decisions.txt` has the full audit trail); original emails untouched in inbox. Fix is on preview only — buggy version still live on main until approved and merged.
...
- preview: `9cfac05` — stage data migration + PipelineView grouping fix (2026-07-11)
