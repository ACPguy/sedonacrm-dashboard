...
**Related Records (`relations.relatedRecords`, migrated 2026-07-21):** fifth migration, single call site (TaskDetail Related Records section, `task_relations` join table, self-referential tasks↔tasks) — confirmed via `grep` for `task_relations`/`searchFilter` that no second call site exists. This is the most nuanced purity call of the five migrations so far — the four registry-adjacent values split two-and-two:
- **`searchFilter` stays local** — it's not a function at all, just a template string (`` `id.neq.${data.id}` ``) evaluated in TaskDetail's scope using the CURRENT record's own id, not the target row. Same treatment as `parentId`.
- **`titleHref` stays local** — unlike Property/Vendor/Tenant Contact's `titleHref` (pure functions of `row` alone), this one reads `data.task_num` from TaskDetail's closure to build the `?from=` back-reference (`/tasks/${data.task_num}`) alongside the target's own `?rt=${row.record_type}` hint. This is the exact navigation fix from commit 4069921 — forcing it into the registry would have no way to reach the current record's own url, so it remains caller-supplied, parameterized per call site.
- **`titleField` IS registry-able** — `` `${getTaskPrefix(row)} — ${row.title}` `` only calls `getTaskPrefix` (a stateless utility from `utils/taskPrefix.js`, no closures), so unlike `taskContacts`' `contactTitle` it has no dependency on component state. Lives in the registry.
- **`iconField` IS registry-able** — `row => ({work_order:Wrench, ...}[row.record_type] || Link)` only reads `row.record_type`. Lives in the registry alongside a static `icon: Link` fallback and `titleTarget: '_self'` (a plain literal, not a function — registry-able the same way `allowCreate`/`icon` are).

Verified the 4069921 navigation behavior survives intact post-migration (mocked-fetch harness): resolved `<a>` href/target/arrow are byte-identical between old and new at every lifecycle stage (initial render, after linking a new record, after unlinking the original) — `target="_self"` (same-tab), no `↗` arrow, and the `?rt=`/`?from=` query values correctly reflect each record's own `record_type` and the current record's own url, not a stale/shared value. `LinkField` and the `Link` icon imports were removed from `TasksView.jsx` as dead code — this migration was the last direct `<LinkField>` JSX usage in that file (the reverseFK Vendor/Tenant Contacts tabs live in `VendorsView.jsx`/`TenantsView.jsx`, not `TasksView.jsx`).

**Vendor Contacts / Tenant Contacts (`relations.vendorContacts` / `relations.tenantContacts`, migrated 2026-07-21):** sixth migration — VendorDetail's and TenantDetail's Contacts tabs (`VendorsView.jsx`/`TenantsView.jsx`), `mode='reverseFK'` (no join table — LinkField queries `contacts` directly by `reverseField=eq.parentId` and PATCHes that FK to link/unlink). One call site each, confirmed via `grep` for `reverseField`. The two configs are identical in every field **except** `reverseField` (`'vendor_id'` vs `'tenant_id'`) and the FK column name inside `linkedFields` — kept as **two separate registry entries** rather than one shared entry with `reverseField` as a caller prop: unifying `linkedFields` to select both FK columns would change the actual query each page issues today (a real behavior change, not a refactor), and `reverseField` is schema-shape config — like `linkedIdField`/`parentIdField` in the join-table entries — not a per-record value like `parentId`, so it's baked into each entry. `mode: 'reverseFK'` itself is also baked into the registry (a per-relationship constant); the call sites no longer pass it. Unlike every prior Vendor/Tenant Contact-adjacent migration, `titleField`/`subtitleField`/`titleHref` here **are** pure functions of `row` with no closures (`row => row.full_name`, etc.) — `VendorsView.jsx`/`TenantsView.jsx` have no TaskDetail-style `contactTitle`/`contactPropCode` helpers, so everything registry-able actually made it into the registry this time. Verified zero-visual-diff including full link/unlink lifecycle (PATCH sets the FK on link, PATCH nulls it on unlink) via mocked-fetch harnesses for both tabs — all matched byte-for-byte.

**Series status:** this closes the six-migration RelationField/relations.js series (Property → Vendor Contact → Tenant Contact → Contacts multi-mode → Related Records → Vendor/Tenant Contacts reverseFK). **Correction to the "zero remaining callers" premise this task was scoped under:** grepping `<LinkField` across the whole codebase after this migration shows it is **not yet fully true** — `ContactsView.jsx` has three additional direct `<LinkField>` call sites that were never part of any of the six migrations and were not previously tracked anywhere in this doc: a Vendor Company picker (single-mode, `vendors` table, ~line 903), a Tenant Company picker (single-mode, `tenants` table, ~line 922), and a read-only reverse-direction "Linked Tasks" display (multi-mode via `task_contacts`, ~line 998). These are unmigrated and out of scope for this session — flagged here rather than silently touched or silently omitted. See Next Priorities.
...
- preview: `41892c3` — refactor: RelationField/relations.js extended to reverseFK Vendor/Tenant Contacts (2026-07-21, not yet merged to main)
